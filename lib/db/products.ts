...
const productsByCategory = normalizedProducts.reduce<Map<string, Product[]>>(
  (result, entry) => {
...
export const getProductBySlug = (title: string): Product | null => {
  ...
  return slug
    ? (products.find((p) => createSlug(p.title) === slug) ?? null)
    : null;
};
...
export const getProductsByPriceRange = (
  ...
  const filtered = products.filter(
    (p) => p.price.withDiscount >= min && p.price.withDiscount <= max,
  );
  return paginate(filtered, page, limit);
};
...
export const getProductsSorted = (
  ...
  const sorted = [...products].sort((a, b) => {
    switch (sortBy) {
      case "price_asc":
        return a.price.withDiscount - b.price.withDiscount;
      case "price_desc":
        return b.price.withDiscount - a.price.withDiscount;
      case "popular":
        return b.performance.sales - a.performance.sales;
      case "rating":
        return (
          (b.performance.ratingAverage ?? 0) -
          (a.performance.ratingAverage ?? 0)
        );
      default:
        return 0;
    }
...
export const getRelatedProducts = (productId: string, limit: number = 10) => {
  ...
  return products
    .filter(
      (p) => p.category === current.category && p.product_id !== productId,
    )
    .slice(0, limit);
};

// Get related products dengan pagination
export const getRelatedProductsPaginated = (
  productId: string,
  page: number = 1,
  limit: number = 10,
) => {
...
export const getFilteredProducts = ({
  ...
  if (minPrice)
    filtered = filtered.filter(
      (entry) => entry.product.price.withDiscount >= minPrice,
    );
  if (maxPrice)
    filtered = filtered.filter(
      (entry) => entry.product.price.withDiscount <= maxPrice,
    );
