...
export type DigitalTransactionStatus = 'pending' | 'completed' | 'failed' | 'cancelled'
...
export const updateDigitalTransaction = async (
  ...
  transactionId: string,
  userId: string,
  updates: Partial<CreateDigitalTransactionPayload> & { status?: DigitalTransactionStatus }
) => {
  ...
  if (updates.paymentMethod) updatePayload.payment_method = updates.paymentMethod
  if (updates.fieldValues) updatePayload.field_values = updates.fieldValues
  if (updates.status) updatePayload.status = updates.status
  if (updates.transactionNotes) updatePayload.transaction_notes = updates.transactionNotes
