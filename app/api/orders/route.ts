...
import { insertNotification } from "@/lib/db/notification";
  ...
  try {
    ...
    const filteredOrders = orders.filter((order) => {
      ...
      const statusMatch = statuses
        ? statuses.has(String(order.status).toLowerCase())
        : true;
    ...
    return NextResponse.json(
      { success: true, data: filteredOrders },
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { products, shippingAddress, notes, paymentMethod, deliveryMethod } =
      body;

    if (!products?.length) {
      return NextResponse.json(
        { success: false, message: "Cart kosong" },
        { status: 400 },
      );
    }
    ...
    if (!shippingAddress) {
      return NextResponse.json(
        { success: false, message: "Alamat pengiriman wajib diisi" },
        { status: 400 },
      );
    }
