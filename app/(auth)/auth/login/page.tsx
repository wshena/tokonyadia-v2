...
const LoginForm = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    getUser(data.user);
    setAlert({
      label: "Login berhasil. Selamat datang kembali!",
      type: "success",
    });
  ...
  return (
    <form onSubmit={handleLogin}>
      <div className="w-75 rounded-[10px] border border-gray-300 p-4 shadow-lg md:w-fit md:p-[1.3rem]">
        <div className="flex w-full flex-col items-center gap-5">
          ...
          <div className="flex w-full items-center justify-between gap-2.5">
            <span className="h-0.5 w-42.5 bg-gray-300" />
          ...
          <button
            type="submit"
            disabled={loading}
            className={`h-12.5 w-full rounded-[10px] border-2 text-center font-bold transition-colors ${
              loading
