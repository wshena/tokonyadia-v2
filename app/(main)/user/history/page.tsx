    ...
    const fetchTransactions = async () => {
      try {
        ...
        const totalSpent = data.reduce(
          (sum: number, tx: DigitalTransaction) => sum + tx.price,
          0,
        );
        const topupCount = data.filter(
          (tx: DigitalTransaction) => tx.category === "topup",
        ).length;
        const tagihanCount = data.filter(
          (tx: DigitalTransaction) => tx.category === "tagihan",
        ).length;
        ...
        setAlert({
          label:
            error instanceof Error
              ? error.message
              : "Gagal memuat history transaksi",
          type: "error",
        });
      } finally {
        setLoading(false);
        ...
        <ContentContainer>
          <div className="rounded-md border border-gray-200 bg-white px-6 py-16 text-center shadow-sm">
            ...
            <p className="mt-3 text-gray-600">
              Anda perlu login untuk melihat history transaksi digital services.
            </p>
            <Link
              href="/auth/login"
            ...
            <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
              <div>
                <h1 className="text-3xl font-bold text-gray-900">
                  History Pembayaran Digital
                </h1>
                <p className="mt-2 text-gray-600">
                  Riwayat lengkap semua transaksi top up dan tagihan Anda
                </p>
              </div>
              <Link
                href="/"
                  ...
                  <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="mt-4 grid gap-3 sm:grid-cols-2 md:grid-cols-auto">
                        {Object.entries(transaction.field_values).map(
                          ([key, value]) => (
                            <div
                              key={key}
