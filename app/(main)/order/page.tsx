...
const statusConfig: Record<OrderStatus, { label: string; className: string; description: string }> = {
  pending: {
    ...
    description: 'Pesanan siap dibayar dan akan diproses setelah pembayaran berhasil.',
  },
  paid: {
    label: 'Sudah Dibayar',
    className: 'border-blue-200 bg-blue-50 text-blue-700',
    description: 'Pembayaran sudah diterima. Sistem sedang menyiapkan pengiriman.',
  },
  shipped: {
...
const OrderPage = () => {
  ...
  const selectAllPending = () => {
    setSelectedOrders(new Set(orders.filter(order => order.status === 'pending').map(order => order.id)))
  }

  const deselectAll = () => setSelectedOrders(new Set())

  const runOrderAction = async (method: 'PATCH' | 'DELETE', ids: string[], successLabel: string, body?: Record<string, string>) => {
      ...
      await Promise.all(
        ids.map(async orderId => {
          ...
          if (!response.ok) {
            ...
            throw new Error(payload?.message ?? `Gagal memproses order ${orderId}`)
          }
        })
      )
  ...
  const proceedToPayment = () => {
    ...
    if (pendingIds.length === 0) {
      setAlert({ label: 'Pilih minimal satu order dengan status pending.', type: 'warning' })
      return
    }
        ...
        <ContentContainer>
          <section className="rounded-[28px] border border-gray-200 bg-white px-6 py-10 text-center shadow-sm md:px-10">
            ...
            <div className="mt-6 flex flex-col justify-center gap-3 sm:flex-row">
              <Link href="/" className="inline-flex cursor-pointer rounded-lg bg-green-600 px-5 py-3 font-medium text-white transition-colors hover:bg-green-700">
                Mulai Belanja
              </Link>
              <Link href="/wishlist" className="inline-flex cursor-pointer rounded-lg border border-green-500 px-5 py-3 font-medium text-green-700 transition-colors hover:bg-green-50">
                Lihat Wishlist
              </Link>
            </div>
          </section>
  ...
  return (
    <main className="w-full pt-10 md:pt-20">
        ...
        <div className="space-y-8">
          <section className="overflow-hidden rounded-[28px] border border-gray-200 bg-white shadow-sm">
            <div className="bg-linear-to-r from-green-600 via-emerald-500 to-lime-400 px-6 py-8 text-white md:px-8">
              <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
                ...
                <div className="flex flex-wrap gap-3">
                  <Link href="/cart" className="inline-flex cursor-pointer rounded-lg bg-white px-4 py-2 font-medium text-green-700 transition-colors hover:bg-green-50">
                    Kembali ke Cart
                  </Link>
                  <Link href="/wishlist" className="inline-flex cursor-pointer rounded-lg border border-white/40 px-4 py-2 font-medium text-white transition-colors hover:bg-white/10">
                    Buka Wishlist
                  </Link>
                </div>
              </div>
            ...
            <div className="grid gap-4 px-6 py-6 md:grid-cols-2 xl:grid-cols-4 md:px-8">
              <div className="rounded-2xl border border-gray-200 bg-gray-50 p-4">
                ...
                <h2 className="mt-2 text-2xl font-bold text-gray-900">{summary.totalOrders}</h2>
              </div>
              <div className="rounded-2xl border border-gray-200 bg-gray-50 p-4">
                <p className="text-sm text-gray-500">Produk dibeli</p>
                <h2 className="mt-2 text-2xl font-bold text-gray-900">{summary.totalItems}</h2>
              </div>
              <div className="rounded-2xl border border-gray-200 bg-gray-50 p-4">
                <p className="text-sm text-gray-500">Menunggu pembayaran</p>
                <h2 className="mt-2 text-2xl font-bold text-gray-900">{summary.pendingOrders}</h2>
              </div>
              <div className="rounded-2xl border border-gray-200 bg-gray-50 p-4">
                <p className="text-sm text-gray-500">Total belanja</p>
                <h2 className="mt-2 text-2xl font-bold text-gray-900">{formatCurrency(orders[0]?.currency ?? 'IDR', summary.totalSpent)}</h2>
              </div>
            </div>
          </section>
          ...
          {selectedOrders.size > 0 && (
            <section className="rounded-2xl border border-blue-200 bg-blue-50 p-4">
              <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <p className="text-sm font-semibold text-blue-900">{selectedOrders.size} order dipilih</p>
                <div className="flex flex-wrap gap-2">
                  <button onClick={proceedToPayment} className="rounded-lg bg-green-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-green-700">
                    Bayar Sekarang
                  </button>
                  <button onClick={() => setModalType('bulk-cancel')} className="rounded-lg bg-red-100 px-4 py-2 text-sm font-medium text-red-700 transition-colors hover:bg-red-200">
                    Batalkan
                  </button>
                  <button onClick={() => setModalType('bulk-delete')} className="rounded-lg bg-gray-100 px-4 py-2 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-200">
                    Hapus
                  </button>
                  <button onClick={deselectAll} className="rounded-lg bg-white px-4 py-2 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-100">
                    Batal Pilih
                  </button>
                </div>
              </div>
          ...
          <section className="space-y-5">
            <div className="flex items-center justify-between rounded-2xl border border-gray-200 bg-white p-4">
              <p className="text-sm text-gray-600">Pilih beberapa order pending untuk dibayar sekaligus.</p>
              <button onClick={selectAllPending} className="rounded-lg border border-green-200 px-4 py-2 text-sm font-medium text-green-700 transition-colors hover:bg-green-50">
                Pilih Semua Pending
              </button>
            </div>
            ...
            {sortedOrders.map(order => {
              ...
              return (
                <article key={order.id} className="rounded-[28px] border border-gray-200 bg-white p-5 shadow-sm md:p-6">
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex flex-wrap gap-x-5 gap-y-2 text-sm text-gray-500">
                          ...
                          <span>{deliveryLabelMap[order.delivery_method] ?? order.delivery_method}</span>
                          <span>{paymentLabelMap[order.payment_method] ?? order.payment_method}</span>
                        </div>
                      </div>
                    </div>

                    <div className="space-y-3 lg:text-right">
                      <p className="text-2xl font-bold text-gray-900">{formatCurrency(order.currency, order.total_price)}</p>
                      <div className="flex flex-wrap gap-2 lg:justify-end">
                        {isPending && (
                          <button onClick={() => router.push(`/payment?orders=${order.id}`)} className="rounded-lg bg-green-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-green-700">
                            Bayar
                          </button>
                        )}
                        {isPending && (
                          <button onClick={() => { setSelectedOrderId(order.id); setModalType('cancel') }} className="rounded-lg bg-red-100 px-4 py-2 text-sm font-medium text-red-700 transition-colors hover:bg-red-200">
                            Batalkan
                          </button>
                        )}
                        <button onClick={() => { setSelectedOrderId(order.id); setModalType('delete') }} className="rounded-lg bg-gray-100 px-4 py-2 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-200">
                          Hapus
                        </button>
                        {(order.status === 'paid' || order.status === 'shipped' || order.status === 'delivered') && (
                          <button onClick={() => router.push(`/shipping?orders=${order.id}`)} className="rounded-lg border border-sky-200 px-4 py-2 text-sm font-medium text-sky-700 transition-colors hover:bg-sky-50">
                            Lacak Pengiriman
                          </button>
                        )}
                      </div>
                  ...
                  <div className="grid gap-4 py-5 xl:grid-cols-[minmax(0,1fr)_320px]">
                    ...
                    <aside className="space-y-4 rounded-2xl border border-gray-100 bg-gray-50 p-4">
                      <div>
                        <p className="text-sm font-medium text-gray-500">Alamat Pengiriman</p>
                        <p className="mt-1 text-sm leading-6 text-gray-800">{order.shipping_address}</p>
                      </div>
                      {order.notes && (
                        <div>
                          <p className="text-sm font-medium text-gray-500">Catatan</p>
                          <p className="mt-1 text-sm leading-6 text-gray-800">{order.notes}</p>
                        </div>
                      )}
                    </aside>
                  </div>
      ...
      <CancelOrderModal
        ...
        onClose={closeModal}
        onConfirm={() => {
          const ids = modalType === 'bulk-cancel' ? Array.from(selectedOrders) : selectedOrderId ? [selectedOrderId] : []
          runOrderAction('PATCH', ids, `${ids.length} pesanan berhasil dibatalkan.`, { status: 'cancelled' })
        }}
      ...
      <DeleteOrderModal
        ...
        onClose={closeModal}
        onConfirm={() => {
          const ids = modalType === 'bulk-delete' ? Array.from(selectedOrders) : selectedOrderId ? [selectedOrderId] : []
          runOrderAction('DELETE', ids, `${ids.length} pesanan berhasil dihapus.`)
        }}
        isProcessing={isProcessing}
