...
const WishlistPage = () => {
  ...
  const handleRemove = async (productId: string) => {
    ...
    try {
      ...
      setAlert({
        label:
          error instanceof Error ? error.message : "Gagal menghapus wishlist.",
        type: "error",
      });
    }
  };
  ...
  if (!user?.id) {
    return (
      <main className="w-full pt-10 md:pt-20">
        <ContentContainer>
          <div className="rounded-md border border-gray-200 bg-white px-6 py-16 text-center shadow-sm">
            <h1 className="text-2xl font-bold text-gray-900">
              Wishlist membutuhkan login
            </h1>
            <p className="mt-3 text-gray-600">
              Masuk dulu agar produk favoritmu bisa disimpan di wishlist.
            </p>
            <Link
              href="/auth/login"
    ...
    <main className="w-full pt-10 md:pt-20">
      <ContentContainer>
        <div className="space-y-8">
            ...
            <div className="relative overflow-hidden bg-linear-to-r from-rose-500 via-orange-400 to-amber-300 px-6 py-8 text-white md:px-8">
              ...
              <div className="relative flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    <h1 className="text-3xl font-bold md:text-4xl">
                      Simpan produk favoritmu di satu tempat
                    </h1>
                    <p className="max-w-2xl text-sm leading-6 text-white/90 md:text-base">
                      Barang impian jangan sampai hilang. Kumpulkan semua yang
                      kamu suka di sini dan bawa pulang saat waktunya tepat!
                    </p>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-3">
                  <Link
                    href="/product/all"
                    className="inline-flex rounded-lg bg-white px-4 py-2 font-medium text-orange-700 transition-colors hover:bg-orange-50"
                  >
                    Jelajahi Produk
                  </Link>
                  {products.some(
                    ...
                    <button
                      onClick={handleClear}
                      className="rounded-lg border border-white/40 px-4 py-2 font-medium text-white transition-colors hover:bg-white/10"
                    >
                      Kosongkan Wishlist
                    </button>
                  )}
          ...
          {isHydrating ? (
            <section className="rounded-md border border-gray-200 bg-white px-6 py-14 text-center shadow-sm">
              <div className="mx-auto h-12 w-12 animate-spin rounded-full border-b-2 border-rose-500" />
              <p className="mt-5 text-gray-600">
                Menyinkronkan wishlist dari Supabase...
              </p>
            </section>
          ) : products.length === 0 ||
            ...
            <section className="rounded-md border border-gray-200 bg-white px-6 py-14 text-center shadow-sm">
              ...
              <p className="mx-auto mt-3 max-w-xl text-gray-600">
                Tambahkan produk dari halaman katalog atau detail produk, lalu
                semua favoritmu akan muncul di sini.
              </p>
              <Link
                href="/product/all"
                className="mt-6 inline-flex rounded-lg bg-green-600 px-5 py-3 font-medium text-white transition-colors hover:bg-green-700"
            ...
            <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
              {products.flatMap((product) => {
                return product?.products?.map((item) => {
                  const price =
                    item?.price?.withDiscount && item.price.withDiscount > 0
                  ...
                  return (
                    ...
                    >
                      <div className="relative mb-5 h-60 overflow-hidden rounded-2xl bg-gray-100">
                        <Image
                          src={
                            item?.images?.["800x900"]?.[0] ??
                            "/image/1-emptystate.png"
                          }
                          alt={item.title}
                          fill
                      ...
                      <div className="space-y-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-500">
                            {item?.performance?.sales ?? 0} terjual
                          </p>
                        </div>
