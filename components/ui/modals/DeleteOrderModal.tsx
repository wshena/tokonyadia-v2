...
const DeleteOrderModal: React.FC<DeleteOrderModalProps> = ({
  ...
  const title = isBulk ? `Hapus ${selectedCount} Pesanan?` : 'Hapus Pesanan?'
  const message = isBulk
    ? 'Semua pesanan yang dipilih akan dihapus secara permanen. Apakah Anda yakin?'
    : 'Pesanan akan dihapus secara permanen dan tidak bisa dipulihkan. Apakah Anda yakin?'
  const confirmText = isBulk ? 'Hapus Semua' : 'Hapus'
