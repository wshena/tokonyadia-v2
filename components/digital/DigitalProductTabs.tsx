...
import { getDigitalServicesByCategory, type DigitalService, type DigitalServiceCategory } from '@/lib/data/digitalServices'
import { useUtilityStore } from '@/lib/zustand/utilityStore'

const categoryLabels: Record<DigitalServiceCategory, { title: string; description: string }> = {
  topup: {
    title: 'Top Up',
...
const DigitalProductTabs = () => {
  ...
  const services = useMemo(() => getDigitalServicesByCategory(activeCategory), [activeCategory])
  const activeService = useMemo<DigitalService | undefined>(
    () => services.find((service) => service.id === activeServiceId) ?? services[0],
    [activeServiceId, services]
  )
  const selectedNominalOption = activeService?.nominalOptions.find((item) => item.value === selectedNominal)
  ...
  const handleSubmit = () => {
    const emptyField = activeService.fields.find((field) => !fieldValues[field.id]?.trim())
    ...
    router.push(`/payment/digital?data=${encodeURIComponent(JSON.stringify(payload))}`)
  }

  return (
    <section className="rounded-4xl border border-gray-200 bg-white p-5 shadow-sm md:p-7">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          <span className="text-sm font-semibold uppercase tracking-[0.22em] text-green-600">Digital Service</span>
          <h2 className="mt-2 text-2xl font-bold text-gray-900 md:text-3xl">Top Up dan Tagihan dalam satu tempat</h2>
          <p className="mt-2 max-w-2xl text-sm text-gray-600 md:text-base">
            Pilih layanan, isi data pelanggan, lalu lanjut ke halaman pembayaran seperti alur checkout produk.
          </p>
        </div>
      ...
      <div className="mt-6 flex gap-3 rounded-2xl bg-gray-100 p-2">
        {(Object.keys(categoryLabels) as DigitalServiceCategory[]).map((category) => {
          ...
          return (
            ...
            >
              <p className="font-semibold">{categoryLabels[category].title}</p>
              <p className="mt-1 text-sm">{categoryLabels[category].description}</p>
            </button>
          )
        })}
      </div>
              ...
              <div className="absolute left-0 top-[calc(100%+0.75rem)] z-20 min-w-60 rounded-2xl border border-gray-200 bg-white p-2 shadow-lg">
                {hiddenServices.map((service) => (
                  ...
                  >
                    ...
                    <span className="mt-1 text-sm text-gray-500">{service.description}</span>
                  </button>
                ))}
              </div>
            )}
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
        <div className="rounded-[28px] border border-gray-200 bg-gray-50 p-5">
          <div className="mb-5">
            <h3 className="text-xl font-semibold text-gray-900">{activeService.formTitle}</h3>
            <p className="mt-1 text-sm text-gray-600">{activeService.description}</p>
          </div>
          ...
          <div className="mt-6">
            <h4 className="text-sm font-semibold uppercase tracking-[0.2em] text-gray-500">Pilih nominal</h4>
        ...
        <aside className="h-fit rounded-[28px] border border-gray-200 bg-white p-5 shadow-sm lg:sticky lg:top-24">
          ...
          <div className="mt-5 space-y-3 rounded-2xl bg-gray-50 p-4 text-sm text-gray-600">
            <div className="flex items-center justify-between">
              <span>Kategori</span>
              <span className="font-medium text-gray-900">{categoryLabels[activeCategory].title}</span>
            </div>
            <div className="flex items-center justify-between">
              <span>Layanan</span>
              <span className="font-medium text-gray-900">{activeService.label}</span>
            </div>
            <div className="flex items-center justify-between">
              <span>{activeService.summaryLabel}</span>
              <span className="max-w-37.5 truncate text-right font-medium text-gray-900">
                {fieldValues[activeService.fields[activeService.fields.length - 1]?.id] || '-'}
              </span>
            </div>
            <div className="border-t border-gray-200 pt-3">
              <div className="flex items-center justify-between text-base font-bold text-gray-900">
                ...
                <span className="text-green-600">
                  {selectedNominalOption ? formatCurrency(selectedNominalOption.price) : '-'}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="mt-5 rounded-2xl border border-green-100 bg-green-50 p-4 text-sm text-green-800">
            Lanjutkan ke halaman pembayaran untuk menyelesaikan proses {activeCategory === 'topup' ? 'top up' : 'tagihan'} ini.
          </div>
